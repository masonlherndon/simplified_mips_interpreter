...
string extract_next_argument(string& line){
    ...
    else if (comma_pos != -1){ // comma exists (at least 2 arguments left)
        next_arg = line.substr(0, comma_pos);
        line.erase(0, comma_pos + 1);
        return next_arg;
    }
    else { // comma does not exist (only 1 argument left)
        next_arg = line.substr(0, comma_pos);
...
string get_register_from_address(const string& str){
    ...
    if (str[0] != 'R'){ // op1 is immediate, op2 is register
        return op2;
    }
    else { // op1 is register, op2 is immediate
        return op1;
    }
}
...
void update_flags(FlagReg& flags, vector<Instruction*>& active_instrs, const vector<Instruction>& running_instrs, const Stage stage){
    ...
    if (active_instrs[ID] && active_instrs[ID]->exists && active_instrs[ID]->opcode == "HLT"){
        flags.program_complete = active_instrs[EX1] && active_instrs[EX1]->has_completed
                                && active_instrs[EX2] && active_instrs[EX2]->has_completed
                                && active_instrs[EX3] && active_instrs[EX3]->has_completed
                                && active_instrs[MEM] && active_instrs[MEM]->has_completed
                                && active_instrs[WB] && active_instrs[WB]->has_completed;
    }
}
...
Instruction get_next_filled_instruction(const vector<Instruction>& instructions, int& PC){
    ...
    else while (PC < instructions.size()){
...
int get_value(const Memory& memory, const vector<bitset<REG_SIZE>>& registers, const string& str){
    ...
    if (VALID_REGISTERS.contains(str)){ // its a register
        return registers.at(VALID_REGISTERS.at(str)).to_ullong();
    }
    else if (left_pos != -1 && right_pos != -1){ // its an address
        return resolve_address(memory.data, registers, str);
    }
    else if (memory.program.labels.contains(str)){ // its a label (happens to handle some immediates)
        return memory.program.labels.at(str);
    }
    else { // its an immediate
